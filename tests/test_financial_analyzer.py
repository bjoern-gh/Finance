...
from financial_analyzer import parse_and_convert_tickers, format_large_number, get_financial_metrics
...
@pytest.mark.parametrize("input_string, expected_output", [
    ...
    ("Some text FRA:R5A more text NASDAQ:GOOGL end", [("FRA:R5A", "R5A.F"), ("NASDAQ:GOOGL", "GOOGL")]),
    ("", []),
...
@patch('financial_analyzer.logging') # Mock logging to prevent console output during tests
def test_get_financial_metrics_success(mock_logging, mock_config, MockTicker):
    ...
    assert result["Trend"] == "BULLISH" # 105 > 104.50 and KGV < KGV_MAX_THRESHOLD (default 25, but mocked to 25)
...
def test_get_financial_metrics_api_error(mock_logging, mock_config, MockTicker):
    ...
    mock_ticker_instance.history.side_effect = Exception("API Limit Exceeded") # Simulate API error
...
def test_get_financial_metrics_no_pe_no_eps(mock_logging, mock_config, MockTicker):
    ...
    assert result["Trend"] == "BULLISH" # Price > SMA, and KGV is N/A (which is treated as not exceeding threshold)
    assert result["Status"] == "OK"
