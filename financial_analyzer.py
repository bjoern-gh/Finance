...
# RAW_DATA wird hier nicht geladen, da es von der aufrufenden Funktion übergeben wird
    ...
    for attempt in range(RETRIES):
        try:
            ...
            if hist.empty or len(hist) < SMA_PERIOD:
                logging.warning(f"[{original_ticker}] Keine/unzureichende Daten für {yahoo_ticker_symbol} (Versuch {attempt + 1}/{RETRIES}).")
                return {
            ...
            if INCLUDE_MARKET_CAP:
                ...
                if mkt_cap is not None:
                    market_cap = format_large_number(mkt_cap) # Hier wird die Formatierungsfunktion angewendet
            ...
            trend = "BULLISH" if is_bullish and (isinstance(pe_v, (int, float)) and pe_v < KGV_MAX_THRESHOLD or pe_v == "N/A") else "HALTEN"
            ...
            logging.error(f"[{original_ticker}] Fehler bei Abruf von {yahoo_ticker_symbol} (Versuch {attempt + 1}/{RETRIES}): {e}")
...
def analyze_tickers(raw_data_string):
    """
    Hauptfunktion zur Analyse einer Liste von Ticker-Symbolen.
    Gibt ein Pandas DataFrame mit den Ergebnissen zurück.
    """
    ticker_tuples = parse_and_convert_tickers(raw_data_string)
    logging.info(f"Analyse von {len(ticker_tuples)} Tickers gestartet (parallelisiert).")

    results = []
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        for res in tqdm(executor.map(get_financial_metrics, ticker_tuples), total=len(ticker_tuples), desc="Verarbeite Ticker"):
            results.append(res)
    ...
    if SORT_BY_COLUMN in df.columns:
        ...
        logging.info(f"Ergebnisse sortiert nach '{SORT_BY_COLUMN}' (aufsteigend: {SORT_ASCENDING}).")
    else:
        logging.warning(f"Sortierspalte '{SORT_BY_COLUMN}' nicht gefunden. Keine Sortierung angewendet.")
